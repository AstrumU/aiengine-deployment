...
prediction_model_path = blobpath + "/Campaign_Recommendation/Prediction_Model/basic_model"
...
def rankingrec():
    ...
    prediction_model = PipelineModel.load(prediction_model_path)
    predictions = prediction_model.transform(campaign_feature_space)
    prediction_result = predictions.select(
        "AstrumU_UUID", extract_prob_udf(col("probability")).alias("Evaluation_Score"))
    campaign_feature_space_evaluation = campaign_feature_space.join(prediction_result, "AstrumU_UUID", "left")
    ...
    campaign_feature_space_evaluation = campaign_feature_space_evaluation \
        .withColumn("rank_number", row_number().over(recommentdation_windowSpec))
    campaign_rec_result = campaign_feature_space_evaluation \
        .select('AstrumU_UUID','Campaign_ID', 'Program_ID', 'Evaluation_Score',
                'Rank_Number', 'Student_Skills', 'Hard_Skill_ranking_list',
                'Soft_Skill_ranking_list').distinct()
    campaign_rec_result.write.format("delta").mode("overwrite") \
        .option("overwriteSchema", "true").save(campaign_rec_result_path)
