...
# 	curl -X POST -F data=@campaign_entity.json 'http://localhost/8003'
...
@app.route("/")
def hello():
    return "Hello from ranking recommenation!"

@app.route("/rankingrec")
def rankingrec():
    ...
    status = f"Success. Generate recommendation result"
    return status
